(function ($) {
    ...
    // $('.dsn-grid-info').find(".dsn-grid-info-wrapper .title , .custom-btn").off('click');
})(jQuery);
    ...
    wind.on("scroll", function () {
        ...
        if (bodyScroll > 250) {
		  ...
		  if (bodyScroll > 250) {
            ...
            $(".secodaryNav").addClass("nav-up");
            

        } else {

            $(".secodaryNav").removeClass("nav-up");
...
function dsn_slider() {
    ...
    function animationLeft() {
        if (!dsn_slider_label.length) {
            return;
        }
        TweenMax.fromTo(dsn_slider_label[0], speed_slide_custom, {left: '100%'}, {ease: Power3.easeOut, left: '-100%'});
    }
    ...
    dsn_slider_current.on('afterChange', function (event, slick, currentSlide) {
        ...
        if (span_num.length) {
            ...
            t2.fromTo(span_num[0], 0.5, {opacity: 0, x: 0}, {opacity: 1, ease: Back.easeIn.config(4), x: -20});
            currentSlide++;
            span_num.text((currentSlide < 10 ? "0" : "") + currentSlide);
            t2.fromTo(span_num[0], 1.5, {x: -20}, {ease: Back.easeOut.config(4), x: 0});
        }
        ...
        var slideMeta = dsn_slider_info.find('.slide-meta li ');
        if (slideMeta.length) {
            t.fromTo(slideMeta.toArray(), 1, {opacity: 0, x: -40}, {
                ease: Back.easeOut.config(1),
                opacity: 1,
                x: 0
            });
        }
        var customBtns = dsn_slider_info.find('.custom-btn');
        if (customBtns.length) {
            t.fromTo(customBtns.toArray(), 1, {opacity: 0, x: -40}, {
    ...
    var progressPath = progress_slider.find('svg path');
    if (progressPath.length) {
        tlm_progress.fromTo(progressPath, 8, {'stroke-dashoffset': 309},
            {
                'stroke-dashoffset': 0,
                onComplete: function () {
...
function hoverPlayVideo() {
    ...
    var circle01 = document.querySelector(".play-circle-01");
    var circle02 = document.querySelector(".play-circle-02");
    if (!button || !circle01 || !circle02) {
        return;
    }

    TweenMax.set(circle01, {
        rotation: 90,
        transformOrigin: "center"
    });

    TweenMax.set(circle02, {
        rotation: -90,
        transformOrigin: "center"
    });
    ...
    button.addEventListener("mouseover", function () { rotateTL.play(); });
    button.addEventListener("mouseleave", function () { rotateTL.reverse(); });
}
...
function dsn_view_project(tlm_progress, progress_slider) {
    ...
    var dsn_info_click = dsn_info.find('.dsn-grid-info-wrapper .title');
    ...
    dsn_info_click.on('click', function (e) {
        ...
        function beforeSend(id) {
            ...
            var e_img = dsn_info_current.find();
            ...
            var staggerEls = [];
            if (dsn_info.length) {
                staggerEls.push(dsn_info[0]);
            }
            if (dsn_info_prev.length) {
                staggerEls.push(dsn_info_prev[0]);
            }
            $('.dsn-grid-nav-box').each(function () {
                staggerEls.push(this);
            });
            tlMoveUp.staggerTo(staggerEls, 1, {
                ease: Back.easeIn.config(1.7),
                y: '-100%',
                opacity: 0
            }).to(header[0], 1, {
                ease: Back.easeIn.config(1.7), width: '100%', left: 0, onStart: function () {
                    if (dsn_info_current.length) {
...
function nexProject() {
    ...
    $('.project-next-box ').on('click', function (e) {
        ...
        var nextH4 = _that.find('.next-project h4');
        var nextTitle = _that.find('.next-title');
        if (nextH4.length) {
            tlNext.to(nextH4.toArray(), 1, object);
        }
        if (nextTitle.length) {
            tlNext.to(nextTitle.toArray(), 1, object, .3);
        }
        tlNext.to(_that[0], 1, {
...
function animation(remove) {
    ...
    if (e.length) {
        TweenMax.fromTo(e.toArray(), 1, {opacity: 1, y: "100%"}, {ease: Back.easeOut.config(1.7), opacity: 1, x: '-50%', y: '-50%'});
    }
    TweenMax.to(document.body, 1, {overflow: ''});
  ...
  -------------------------------------------------------*/
    var app = function () {
        ...
        var applyListeners = function applyListeners() {
            // Mobile menu toggle is handled in React (Header.jsx)
        };
